...
class TestLogicFunctionCalculator(unittest.TestCase):
    ...
    def test_process_expression_invalid_chars(self):
        # The implementation checks for lowercase a-e and specific operators
        invalid_exprs = [
            "a∧f",  # invalid lowercase
            "a∧g",
            "a∧z",
            "a∧A",  # uppercase
            "a∧B",  # uppercase
...
def test_eval_error_handling(self):
    # некорректное выражение, вызовет исключение внутри eval
    expr = "a∧(b"
    # eval упадёт, result будет принудительно 0
...
def test_generate_normal_forms_all_zero_all_one(self):
    ...
    self.assertEqual(sdnf, '0')  # Нулевая функция
    ...
    self.assertEqual(sknf, '1')  # Единичная функция
